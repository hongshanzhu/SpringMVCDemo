  ...
  <div class="list2">
    <div class="padC">
      <div style="border:1px solid #C5DDF1; margin-bottom: 10px;padding:10px;">
        ...
        <td> <input type="text" id="no" name="no" value="${fn:escapeXml(agrSpace.no) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">场地名称：</td>
        <td> <input type="text" id="name" name="name" value="${fn:escapeXml(agrSpace.name) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">删除状态：</td>
        <td> <input type="text" id="deleted" name="deleted" value="${fn:escapeXml(agrSpace.deleted) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">状态：</td>
        <td> <input type="text" id="status" name="status" value="${fn:escapeXml(agrSpace.status) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">修改时间：</td>
        <td> <input type="text" id="updateTime" name="updateTime" value="<fmt:formatDate value="${agrSpace.updateTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrSpace.userIdUpdate}'>
        <option value='${agrSpace.userIdUpdate}'>${agrSpace.userUpdate.name}</option>
        </c:if>
        <c:if test='${empty agrSpace.userIdUpdate}'>
        ...
        <td> <input type="text" id="createTime" name="createTime" value="<fmt:formatDate value="${agrSpace.createTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrSpace.userIdCreate}'>
        <option value='${agrSpace.userIdCreate}'>${agrSpace.userCreate.name}</option>
        </c:if>
        <c:if test='${empty agrSpace.userIdCreate}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
...
<script type="text/javascript">
        ...
        $.post("${ROOT}/resCenter/agrSpace/save.do",data,function(j){
        alert(j.msg);
        if(j.code == 1){
        agrSpaceId = j.agrSpaceId;
        gobackList();}},"json");}
