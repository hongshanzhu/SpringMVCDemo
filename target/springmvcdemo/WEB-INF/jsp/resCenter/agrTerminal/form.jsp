  ...
  <div class="list2">
    <div class="padC">
      <div style="border:1px solid #C5DDF1; margin-bottom: 10px;padding:10px;">
        ...
        <td align="right" width="80">终端编码：</td>
        <td> <input type="text" id="no" name="no" value="${fn:escapeXml(agrTerminal.no) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrTerminal.agrSpaceId}'>
        <option value='${agrTerminal.agrSpaceId}'>${agrTerminal.agrSpace.name}</option>
        </c:if>
        <c:if test='${empty agrTerminal.agrSpaceId}'>
        ...
        <td> <input type="text" id="mac" name="mac" value="${fn:escapeXml(agrTerminal.mac) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">终端IP地址：</td>
        <td> <input type="text" id="ip" name="ip" value="${fn:escapeXml(agrTerminal.ip) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">终端名称：</td>
        <td> <input type="text" id="name" name="name" value="${fn:escapeXml(agrTerminal.name) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">删除状态：</td>
        <td> <input type="text" id="deleted" name="deleted" value="${fn:escapeXml(agrTerminal.deleted) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">状态：</td>
        <td> <input type="text" id="status" name="status" value="${fn:escapeXml(agrTerminal.status) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">修改时间：</td>
        <td> <input type="text" id="updateTime" name="updateTime" value="<fmt:formatDate value="${agrTerminal.updateTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrTerminal.userIdUpdate}'>
        <option value='${agrTerminal.userIdUpdate}'>${agrTerminal.userUpdate.name}</option>
        </c:if>
        <c:if test='${empty agrTerminal.userIdUpdate}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
        ...
        <td> <input type="text" id="createTime" name="createTime" value="<fmt:formatDate value="${agrTerminal.createTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrTerminal.userIdCreate}'>
        <option value='${agrTerminal.userIdCreate}'>${agrTerminal.userCreate.name}</option>
        </c:if>
        <c:if test='${empty agrTerminal.userIdCreate}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
...
<script type="text/javascript">
          var agrTerminalId = "${agrTerminal.id}";
        function save(){
        var data = {};
        data.id = agrTerminalId;
        data.no = $("#no").val();
        ...
        $.post("${ROOT}/resCenter/agrTerminal/save.do",data,function(j){
        alert(j.msg);
        if(j.code == 1){
        agrTerminalId = j.agrTerminalId;
        gobackList();}},"json");}
