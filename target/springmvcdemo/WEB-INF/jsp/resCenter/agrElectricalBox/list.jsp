...
<div class="content">
  <div class="list">
        ...
        电箱ID:<input name="id" value="${id}" class="inputText" style="width:50px"/>
        电箱名称:<input name="name" value="${name}" class="inputText" style="width:50px"/>
        ...
        电箱类型ID:<div style='display: inline-block'>
        <input type='hidden' id='agrElectricalBoxTypeName' name='agrElectricalBoxTypeName' value='${agrElectricalBoxTypeName}'/>
        <select id='agrElectricalBoxTypeId' name='agrElectricalBoxTypeId' style='width: 100%;'>
        <c:if test='${not empty agrElectricalBoxTypeId}'>
        <option value='${agrElectricalBoxTypeId}'>${agrElectricalBoxTypeName}</option>
        </c:if>
        <c:if test='${empty agrElectricalBoxTypeIdId}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
        ...
        所属场地:<div style='display: inline-block'>
        <input type='hidden' id='agrSpaceName' name='agrSpaceName' value='${agrSpaceName}'/>
        <select id='agrSpaceId' name='agrSpaceId' style='width: 100%;'>
        ...
        电箱状态:<div style='display: inline-block'>
        <input type='hidden' id='agrElectricalBoxStateName' name='agrElectricalBoxStateName' value='${agrElectricalBoxStateName}'/>
        <select id='agrElectricalBoxStateId' name='agrElectricalBoxStateId' style='width: 100%;'>
        <c:if test='${not empty agrElectricalBoxStateId}'>
        <option value='${agrElectricalBoxStateId}'>${agrElectricalBoxStateName}</option>
        </c:if>
        <c:if test='${empty agrElectricalBoxStateIdId}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
        ...
        <td width="7%" align="center">${o.id}</td>
        <td width="7%" align="center">${o.name}</td>
        <td width="7%" align="center">${o.agrTerminal.name}</td>
        <td width="7%" align="center">${o.agrElectricalBoxType.name}</td>
        <td width="7%" align="center">${o.agrSpace.name}</td>
        <td width="7%" align="center">${o.agrElectricalBoxState.name}</td>
        <td width="7%" align="center">${o.deleted}</td>
        <td width="7%" align="center">${o.status}</td>
        <td width="7%" align="center"><fmt:formatDate value="${o.updateTime }" pattern="yyyy-MM-dd HH:mm"/>
        <td width="7%" align="center">${o.userUpdate.name}</td>
        <td width="7%" align="center"><fmt:formatDate value="${o.createTime }" pattern="yyyy-MM-dd HH:mm"/>
        <td width="7%" align="center">${o.userCreate.name}</td>
        <td width="7%" align="center">
        <a href="${ROOT}/resCenter/agrElectricalBox/form.do?id=${o.id}">修改</a>
    ...
    <script>
      ...
      function create(){
        location.href="${ROOT}/resCenter/agrElectricalBox/form.do";
      }
      ...
      function doRemove(ids){
        ...
        $.post("${ROOT}/resCenter/agrElectricalBox/delete.do",data,function(j){
          alert(j.msg);
          if(j.code == 1){
            Func.reloadPage();
          }
