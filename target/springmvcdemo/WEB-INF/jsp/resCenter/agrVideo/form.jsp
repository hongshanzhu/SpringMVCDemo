  ...
  <div class="list2">
    <div class="padC">
      <div style="border:1px solid #C5DDF1; margin-bottom: 10px;padding:10px;">
        ...
        <td align="right" width="80">视频编码：</td>
        <td> <input type="text" id="no" name="no" value="${fn:escapeXml(agrVideo.no) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrVideo.agrSpaceId}'>
        <option value='${agrVideo.agrSpaceId}'>${agrVideo.agrSpace.name}</option>
        </c:if>
        <c:if test='${empty agrVideo.agrSpaceId}'>
        ...
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">监控名称：</td>
        <td> <input type="text" id="name" name="name" value="${fn:escapeXml(agrVideo.name) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">播放地址：</td>
        <td> <input type="text" id="url" name="url" value="${fn:escapeXml(agrVideo.url) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">删除状态：</td>
        <td> <input type="text" id="deleted" name="deleted" value="${fn:escapeXml(agrVideo.deleted) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">状态：</td>
        <td> <input type="text" id="status" name="status" value="${fn:escapeXml(agrVideo.status) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">修改时间：</td>
        <td> <input type="text" id="updateTime" name="updateTime" value="<fmt:formatDate value="${agrVideo.updateTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrVideo.userIdUpdate}'>
        <option value='${agrVideo.userIdUpdate}'>${agrVideo.userUpdate.name}</option>
        </c:if>
        <c:if test='${empty agrVideo.userIdUpdate}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
        ...
        <td> <input type="text" id="createTime" name="createTime" value="<fmt:formatDate value="${agrVideo.createTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrVideo.userIdCreate}'>
        <option value='${agrVideo.userIdCreate}'>${agrVideo.userCreate.name}</option>
        </c:if>
        <c:if test='${empty agrVideo.userIdCreate}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
...
<script type="text/javascript">
        ...
        $.post("${ROOT}/resCenter/agrVideo/save.do",data,function(j){
        alert(j.msg);
        if(j.code == 1){
        agrVideoId = j.agrVideoId;
        gobackList();}},"json");}
