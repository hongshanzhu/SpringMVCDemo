...
<head>
  ...
  <title>AgrElectricalBoxState</title>
  ...
  <div class="list2">
    <div class="padC">
      <div style="border:1px solid #C5DDF1; margin-bottom: 10px;padding:10px;">
        ...
        <td> <input type="text" id="name" name="name" value="${fn:escapeXml(agrElectricalBoxState.name) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">删除状态：</td>
        <td> <input type="text" id="deleted" name="deleted" value="${fn:escapeXml(agrElectricalBoxState.deleted) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">状态：</td>
        <td> <input type="text" id="status" name="status" value="${fn:escapeXml(agrElectricalBoxState.status) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">修改时间：</td>
        <td> <input type="text" id="updateTime" name="updateTime" value="<fmt:formatDate value="${agrElectricalBoxState.updateTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrElectricalBoxState.userIdUpdate}'>
        <option value='${agrElectricalBoxState.userIdUpdate}'>${agrElectricalBoxState.userUpdate.name}</option>
        </c:if>
        <c:if test='${empty agrElectricalBoxState.userIdUpdate}'>
        ...
        <td> <input type="text" id="createTime" name="createTime" value="<fmt:formatDate value="${agrElectricalBoxState.createTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrElectricalBoxState.userIdCreate}'>
        <option value='${agrElectricalBoxState.userIdCreate}'>${agrElectricalBoxState.userCreate.name}</option>
        </c:if>
        <c:if test='${empty agrElectricalBoxState.userIdCreate}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
...
<script type="text/javascript">
          var agrElectricalBoxStateId = "${agrElectricalBoxState.id}";
        function save(){
        var data = {};
        data.id = agrElectricalBoxStateId;
        data.name = $("#name").val();
        ...
        $.post("${ROOT}/resCenter/agrElectricalBoxState/save.do",data,function(j){
        alert(j.msg);
        if(j.code == 1){
        agrElectricalBoxStateId = j.agrElectricalBoxStateId;
        gobackList();}},"json");}
