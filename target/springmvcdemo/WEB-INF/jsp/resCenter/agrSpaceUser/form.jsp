  ...
  <div class="list2">
    <div class="padC">
      <div style="border:1px solid #C5DDF1; margin-bottom: 10px;padding:10px;">
        ...
        <td align="right" width="80">人员编码：</td>
        <td><div style='display: inline-block;margin-bottom:5px;'>
        <input type='hidden' id='userName' name='userName' value='${userName}'/>
        <select id='userId' name='userId' style='width: 200px;'>
        <c:if test='${not empty agrSpaceUser.userId}'>
        <option value='${agrSpaceUser.userId}'>${agrSpaceUser.user.name}</option>
        </c:if>
        <c:if test='${empty agrSpaceUser.userId}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
        function userIdDataParser(data, selected) {
        	retval = [ { val: '-1' , text: '全部' } ];
        	list=data.result;
        	list.forEach(function(v){retval.push({ val: v.id , text: v.name });});
        	return retval;
        }
        $('#userId').tinyselect({ dataUrl: '/auth/user/getUserListJson.do' , dataParser: userIdDataParser});
        ...
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">场地编码：</td>
        <td><div style='display: inline-block;margin-bottom:5px;'>
        ...
        <c:if test='${not empty agrSpaceUser.agrSpaceId}'>
        <option value='${agrSpaceUser.agrSpaceId}'>${agrSpaceUser.agrSpace.name}</option>
        </c:if>
        <c:if test='${empty agrSpaceUser.agrSpaceId}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
        ...
        <td> <input type="text" id="deleted" name="deleted" value="${fn:escapeXml(agrSpaceUser.deleted) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">状态：</td>
        <td> <input type="text" id="status" name="status" value="${fn:escapeXml(agrSpaceUser.status) }" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        <td align="right" width="80">修改时间：</td>
        <td> <input type="text" id="updateTime" name="updateTime" value="<fmt:formatDate value="${agrSpaceUser.updateTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrSpaceUser.userIdUpdate}'>
        <option value='${agrSpaceUser.userIdUpdate}'>${agrSpaceUser.userUpdate.name}</option>
        </c:if>
        <c:if test='${empty agrSpaceUser.userIdUpdate}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
        ...
        <td> <input type="text" id="createTime" name="createTime" value="<fmt:formatDate value="${agrSpaceUser.createTime }" pattern="yyyy-MM-dd HH:mm"/>" class="inputText" style="width:500px;height:18px;"/></td>
        </tr>
        <tr class="baseinfo" style="margin:10px 0">
        ...
        <c:if test='${not empty agrSpaceUser.userIdCreate}'>
        <option value='${agrSpaceUser.userIdCreate}'>${agrSpaceUser.userCreate.name}</option>
        </c:if>
        <c:if test='${empty agrSpaceUser.userIdCreate}'>
        <option value='-1'>全部</option>
        </c:if>
        </select>
        <script>
...
<script type="text/javascript">
          var agrSpaceUserId = "${agrSpaceUser.id}";
        function save(){
        var data = {};
        data.id = agrSpaceUserId;
        ...
        $.post("${ROOT}/resCenter/agrSpaceUser/save.do",data,function(j){
        alert(j.msg);
        if(j.code == 1){
        agrSpaceUserId = j.agrSpaceUserId;
        gobackList();}},"json");}
