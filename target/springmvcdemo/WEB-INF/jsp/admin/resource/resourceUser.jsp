...
List<UserResource> userResources = resourceService.listResourceUsers(rid);
req.attr("userResources", userResources);
...
<div class="dwz">您现在的位置：用户资源设置</div>
<div class="list">
<div class="listlo">
<b>${resource.name }</b> - 受权用户列表  <input type="button" value="返 回" class="but1" onclick="history.back()">
</div>
<table width="100%" border="0" cellspacing="1" cellpadding="0" class="listtb">      	
  ...
  <c:forEach var="o" items="${userResources}" varStatus="idx"> 
  <tr class="${idx.index%2==0?"bluetr":"" }">
      <td width="30%" align="center">${o.user.truename}</td>
      ...
      <td width="10%" align="center">
		<a href="/admin/resource/read/userResource.jsp?uid=${o.uid }">修改用户权限</a>
		</td>     
  </tr>
  </c:forEach>
</table>
