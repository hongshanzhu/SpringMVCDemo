...
<%
  ...
  if (uri.indexOf("/WEB-INF/jsp") != -1) {
    uri = uri.replace("/WEB-INF/jsp", "").replace(".jsp", ".do");
  }
...
//System.out.println("----------------url="+url);
  ...
  if (pageTotal <= 1){
    sb.append("<div class='dPages'>记录数:<span>"+total+"</span>");
  ...
  sb.append("<div class='dPages'>");
  if(pageNo > 1){
    sb.append("<a class='prev' href='").append(url).append("&pageNo="+(pageNo - 1)).append("'>上一页</a> ");
  }else{
    sb.append("<a class='prev' onclick='return false'>上一页</a> ");
  }
  ...
  if(pageNo < pageTotal){
    sb.append("<a class='next' href='").append(url).append("&pageNo="+(pageNo + 1)).append("'>下一页</a> ");
  }else{
    sb.append("<a class='next' onclick='return false'>下一页</a> ");
  }
  sb.append("记录数:<span>"+total+"</span> 第<input name='pageNo' value='"+pageNo+"' size='3' onkeypress='if(event.keyCode == 13) go2page(this.form)'>页 每页<input name='pageSize' value='"+pageSize+"' size='3' onkeypress='if(event.keyCode == 13) go2page(this.form)'>条");
  sb.append("</div>");
  sb.append("</form>");
